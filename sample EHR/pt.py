...
def update(id):
    if request.method=="GET":
        ...
        chief_complaints=request.form.get("chief_complaints")
        diagnosis=request.form.get("diagnosis")
        patient=conn.execute('update patients set name= ?,age= ?,gender= ?,comorbidities= ?,chief_complaints= ?,diagnosis= ? where id= ?',(name,age,gender,comorbidities,chief_complaints,diagnosis,id)).fetchall()
        conn.commit()
        conn.close()
        return redirect(url_for('pt_record'))
